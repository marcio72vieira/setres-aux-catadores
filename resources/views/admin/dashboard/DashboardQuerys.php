...
class DashboardController extends Controller
{
    ...
    public function ajaxgetCompanhiasMunicipio(Request $request)
    {
        ...
        $residuosdescricao = DB::table('residuos')->rightJoinSub($residuostotal, 'aliasResiduostotal', function ($join) {
            $join->on('residuos.id', '=', 'aliasResiduostotal.idResiduocompanhiaresiduo')
                 ->join('companhia_residuo', 'companhia_residuo.companhia_id', '=', 'aliasResiduostotal.idCompanhia');
        })->select(DB::raw('aliasResiduostotal.idCompanhia, aliasResiduostotal.companhia_nome, aliasResiduostotal.companhia_tipo, aliasResiduostotal.companhia_totalcatadores,
                            aliasResiduostotal.companhia_totalmasc, aliasResiduostotal.companhia_totalfeme,
                            aliasResiduostotal.companhia_totalcomcarteira, aliasResiduostotal.companhia_totalsemcarteira,
                            aliasResiduostotal.pontocoleta_total,
                            aliasResiduostotal.residuo_total,
                            GROUP_CONCAT(residuos.nome SEPARATOR ", ") AS nomeResiduo'))
                    ->groupBy('companhia_residuo.companhia_id')->get();
        ...
        $residuosdescricao = DB::table('residuos')->rightJoinSub($residuostotal, 'aliasResiduostotal', function ($join) {
            $join->on('residuos.id', '=', 'aliasResiduostotal.idResiduocompanhiaresiduo');
                            ...
                            aliasResiduostotal.residuo_total,
                            GROUP_CONCAT(residuos.nome SEPARATOR ", ") AS nomeResiduo'))
                    ->groupBy('aliasResiduostotal.idResiduocompanhiaresiduo')->get();
         */
...
$residuosdescricao =    DB::table('companhias')
->join('municipios', 'municipios.id', '=', 'companhias.municipio_id')
->leftJoin('associados', 'associados.companhia_id', '=', 'companhias.id')
...
->groupBy('companhias.id')
->orderBy('companhias.nome')
->get();


/*
TRAZ O PONTO DE COLETA CORRETAMENTE
 $residuosdescricao =    DB::table('companhias')
                                    ->join('municipios', 'municipios.id', '=', 'companhias.municipio_id')
                                    ->leftJoin('associados', 'associados.companhia_id', '=', 'companhias.id')
                                    ->leftJoin('pontocoletas', 'pontocoletas.companhia_id', '=', 'companhias.id')
...
/** FAZER AS CONSULTAS INVERSA COM SUBQUERYS, OU SEJA, PRIMERIO BUSCA PONTO DE COLETAS, DEPOIS COMPANHIA_RESIDUOS E POR ÚLTIMO COMPANHIAS E ASSOCIADOS */
...
$detalhespontocoletas = DB::table('pontocoletas')->rightJoinSub($detalhesresiduos, 'aliasResiduos', function ($join) {
                            $join->on('pontocoletas.companhia_id', '=', 'aliasResiduos.idCompanhia');
                                    ...
                                    COUNT(DISTINCT pontocoletas.id) AS pontocoleta_total'))
                        ->groupBy('aliasResiduos.idCompanhia')->get();



$data['dados'] =  $detalhespontocoletas;
return response()->json($data);


/****************************************/
/* COPIA ANTIGA DashboardController.php  */
/*****************************************/
<?php
...
class DashboardController extends Controller
{
    ...
    public function ajaxgetCompanhiasMunicipio(Request $request)
    {
        ...
        */
        ...
        /* $residuosdescricao = DB::table('companhia_residuo')->rightJoinSub($residuostotal, 'aliasResiduostotal', function ($join) {
            $join->on('companhia_residuo.companhia_id', '=', 'aliasResiduostotal.idCompanhiacompanhiaresiduo')
                 ->join('residuos', 'residuos.id', '=', 'aliasResiduostotal.idResiduocompanhiaresiduo');
        })->select(DB::raw('aliasResiduostotal.idCompanhia, aliasResiduostotal.companhia_nome, aliasResiduostotal.companhia_tipo, aliasResiduostotal.companhia_totalcatadores,
                            aliasResiduostotal.companhia_totalmasc, aliasResiduostotal.companhia_totalfeme,
                            aliasResiduostotal.companhia_totalcomcarteira, aliasResiduostotal.companhia_totalsemcarteira,
                            aliasResiduostotal.pontocoleta_total,
                            ...
                            residuos.nome,
                            GROUP_CONCAT(residuos.nome SEPARATOR ", ") AS nomeResiduo'))
                    ->groupBy('aliasResiduostotal.idCompanhiacompanhiaresiduo')->get(); */
